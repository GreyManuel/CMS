    ...
    <div id="page-content">
        ...
        <div class="panel rounded-top" style="background-color: #e8ddd3;">
            <div class="panel-heading card-block text-center">
                <h1 class="panel-title text-primary text-bold">List of Members In The {{\Auth::user()->branchname}}</h1>
            </div>
...
$(document).ready(function () {
    ...
    users_table = $('#users-table').DataTable({
        ...
        columns: [
            ...
            { title: "Status", data: {member_status: 'member_status', id: 'id', firstname: 'firstname', lastname: 'lastname'}, name: 'member_status',
             render: (data) => (`${data.member_status == 'active' ? 'Active ' : 'Suspended '}
             ${data.member_status == 'actve' ? '<button value="' + data.id + '" onClick="makeMember(this, users_table.ajax.reload)" class="btn-info" data-placement="up" title="Make ' + data.firstname + ' ' + data.lastname +
            ...
            { title: "Number", data: 'phone', name: 'phone' },
            ...
            { title: "Role", data: 'position', name: 'position' },
            { title: "Altar", data: 'address', name: 'address' },
    ...
    $('#users-table').on( 'click', 'tbody tr td .edit ', function (e) {
      ...
      columns = $(this).parent().closest('tr').find('td').each(function(){
        if (i == 3) {
          ...
          value = ($(this).text() === 'Member') ? 'active' : 'suspended';
            $(this).html(`
              ...
              <div class="col-md-9">
                <select name="position" class="selectpicker col-xs-6 col-sm-4 col-md-6 col-lg-9" data-style="btn-success">
                  ...
                  <option value="violinist">Violinist</option>
                      ...
                      <option value="security">Worship Security</option>
                      <option value="data admin">Data Admin</option>
                      <option value="hospitality">Worship Hospitality</option>
                      <option value="shift leader w">Shift Leader Worship</option>
                      <option value="shift leader k">Shift Leader Keyboard</option>
                      <option value="team leader v">Team Leader Violinist</option>
                      <option value="medic">Worship Medic</option>
                </select>
