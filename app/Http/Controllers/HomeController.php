...
class HomeController extends Controller
{
    ...
    public function index()
    {
      if (Setting::notSet()) {
        ...
        $num_pastors = $user->isAdmin() ? DB::table('members')->where('position', 'keyboardist')->count() : DB::table('members')->where('position', 'keyboardist')->orWhere('position', 'senior pastor')->where('branch_id', \Auth::user()->id)->count();
        $num_workers = $user->isAdmin() ? DB::table('members')->where('position', 'violinist')->count() : DB::table('members')->where('position', 'violinist')->where('branch_id', \Auth::user()->id)->count();
        $num_leaders = $user->isAdmin() ? DB::table('members')->where('position', 'leader')->count() : DB::table('members')->where('position', 'leader')->where('branch_id', \Auth::user()->id)->count();
        $num_sax = $user->isAdmin() ? DB::table('members')->where('position', 'saxophonist')->count() : DB::table('members')->where('position', 'saxophonist')->where('branch_id', \Auth::user()->id)->count();
        $gender_male = $user->isAdmin() ? DB::table('members')->where('sex', 'male')->count() : DB::table('members')->where('sex', 'male')->where('branch_id', \Auth::user()->id)->count();
        $gender_female = $user->isAdmin() ? DB::table('members')->where('sex', 'female')->count() : DB::table('members')->where('sex', 'female')->where('branch_id', \Auth::user()->id)->count();
        $total = ['saxophonist' => $num_sax, 'female' => $gender_female, 'male' => $gender_male, 'leader' => $num_leaders, 'violinist' => $num_workers, 'keyboardist' => $num_pastors, 'members' => $num_members];
